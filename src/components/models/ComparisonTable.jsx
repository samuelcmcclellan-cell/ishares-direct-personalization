        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#E5E5DD]">
              <th className="text-left py-4 px-5 text-xs font-semibold uppercase tracking-wider text-[#7A7A7A] w-48">
                Dimension
              </th>
                ...
                <td key={model.id} className="py-3.5 px-4">
                  <div className="flex flex-wrap gap-1">
                    {model.etfTypes.map((type, i) => (
                      <span key={i} className="text-xs text-[#4A4A4A]">
                        {type}{i < model.etfTypes.length - 1 ? ',' : ''}
                      </span>
                    ))}
                  </div>
                </td>
